...
import { contractABI, contractAddress } from './constants/tokenpresale';
...
async function buyToken(amount, tokenAmount) {
  return new Promise(async (resolve, reject) => {
      try {
          if (!getIsConnected()) reject({reason: 'Wallet not connected'});;
      
          const parsedAmount = parseEther(amount.toString())
          const parsedTokenAmount = parseEther(tokenAmount.toString());
    
          const contract = await  getEthereumContract();

          const options = {value: parsedAmount}
          const buyHash = await contract.buyTokens(parsedTokenAmount, options)
